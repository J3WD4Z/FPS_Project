Shader "Hidden/UVFree/Terrain/StandardSpecular-AddPass" {
	...
	SubShader {
		...
		#pragma surface surf StandardSpecular decal:add vertex:SplatmapVert finalcolor:myfinal exclude_path:prepass exclude_path:deferred
		...
		void SplatmapVert(inout appdata_full v, out Input o)
		{
			...
			v.tangent.xyz = 
				cross(v.normal, mul(_World2Object,fixed4(0.0,sign(worldNormal.x),0.0,0.0)).xyz * (powerNormal.x))
			  + cross(v.normal, mul(_World2Object,fixed4(0.0,0.0,sign(worldNormal.y),0.0)).xyz * (powerNormal.y))
			  + cross(v.normal, mul(_World2Object,fixed4(0.0,sign(worldNormal.z),0.0,0.0)).xyz * (powerNormal.z))
			;
