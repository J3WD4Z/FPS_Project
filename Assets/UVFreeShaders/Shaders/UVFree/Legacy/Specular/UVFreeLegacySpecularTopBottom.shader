Shader "UVFree/Legacy/SpecularTopBottom" {
	Properties {
		...
		_TopShininess("Shininess", Range(0.0, 1.0)) = 0.078125
		...
		_BottomShininess("Shininess", Range(0.0, 1.0)) = 0.078125
	...
	SubShader {
		...
		void surf (Input IN, inout SurfaceOutput o) {
			...
			fixed sgY = _Shininess;
			fixed sgTop = _TopShininess;
			
			#ifdef _UVFREE_BOTTOM
				fixed sgBottom = _BottomShininess;
			#endif
			...
			fixed sg = 
			    lerp(_Shininess * IN.powerNormal.x, sgTop * weightedPowerNormal.x, yLerp)
			  + lerp(sgY * IN.powerNormal.y, sgTop * weightedPowerNormal.y, yLerp)
			  + lerp(_Shininess * IN.powerNormal.z, sgTop * weightedPowerNormal.z, yLerp)
			;
