	...
	SubShader {
		...
		void vert (inout appdata_full v, out Input o) {
			...
			UNITY_INITIALIZE_OUTPUT(Input,o);
				...
				v.tangent.xyz = 
					cross(v.normal, mul(_World2Object,fixed4(0.0,sign(worldNormal.x),0.0,0.0)).xyz * (o.powerNormal.x))
				  + cross(v.normal, mul(_World2Object,fixed4(0.0,0.0,sign(worldNormal.y),0.0)).xyz * (o.powerNormal.y))
				  + cross(v.normal, mul(_World2Object,fixed4(0.0,sign(worldNormal.z),0.0,0.0)).xyz * (o.powerNormal.z))
				;
		...
		void surf (Input IN, inout SurfaceOutput o) {
			...
			o.Albedo = max(fixed3(0.001,0.001,0.001), tex.rgb * _Color.rgb);
			o.Alpha = 1.0;
