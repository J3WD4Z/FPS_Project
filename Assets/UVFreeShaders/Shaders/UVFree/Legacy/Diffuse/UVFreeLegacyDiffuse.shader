	...
	SubShader {
		...
		void vert (inout appdata_full v, out Input o) {
			...
			UNITY_INITIALIZE_OUTPUT(Input,o);
				...
				v.tangent.xyz = 
					cross(v.normal, mul(_World2Object,fixed4(0.0,sign(worldNormal.x),0.0,0.0)).xyz * (nPowerNormal.x))
				  + cross(v.normal, mul(_World2Object,fixed4(0.0,0.0,sign(worldNormal.y),0.0)).xyz * (nPowerNormal.y))
				  + cross(v.normal, mul(_World2Object,fixed4(0.0,sign(worldNormal.z),0.0,0.0)).xyz * (nPowerNormal.z))
				;
